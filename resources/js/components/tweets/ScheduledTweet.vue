<template>
    <div ref="scheduledTweet" class="border-b border-gray py-3 px-3 hover:bg-gray-50 transition">
        <div class="flex">
            <div class="w-0 mr-2 flex justify-center items-center overflow-hidden" :class="{ 'w-10': $store.state.onEdit }">
                <input type="checkbox" class="h-4 w-4 text-blue focus:ring-blue-500 border-gray-300 rounded cursor-pointer" @click="$store.commit('setSelectedScheduledTweets', payload.id)" :checked="$store.state.selectedScheduledTweets.includes(payload.id)" />
            </div>
            <div class="flex-1">
